      ...
      create: function (context) {
        ...
        return {
          CallExpression(node) {
            ...
            // EARLY EXIT: If the function is known to return void (TypeScript), don't report an error
            if (checker && parserServices) {
              ...
              if (signature) {
                ...
                // Exit early if return type is void
                if (checker.typeToString(returnType) === 'void') return;

                const decl = signature.declaration;
                if (decl && decl.body && decl.body.statements) {
                  const hasUsefulReturn = decl.body.statements.some(stmt => {
                    return (
                  ...
                  if (!hasUsefulReturn) {
                    return; // Allow: function doesn't really return anything meaningful
                  }
                }
              }
            }
