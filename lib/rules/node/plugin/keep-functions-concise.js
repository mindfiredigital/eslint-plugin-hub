      ...
      create(context) {
        ...
        function checkFunction(node) {
          if (node.body.type !== 'BlockStatement') {
            if (maxLines === 0) {
              // only report if maxLines is 0 for concise body
              context.report({
                node,
          ...
          // Lines within the curly braces. loc.start.line is the line of '{', loc.end.line is line of '}'.
          ...
          if (startLineIndex <= endLineIndex + 1) {
            // +1 because endLineIndex is 0-based index of last content line
            for (let i = startLineIndex; i <= endLineIndex; i++) {
              // Iterate 0-based indices from sourceCode.getLines()
              ...
              if (skipComments) {
                // Check if the trimmed line is empty or consists only of a comment
                if (
                  trimmedLine === '' ||
                  trimmedLine.startsWith('//') ||
                  (trimmedLine.startsWith('/*') && trimmedLine.endsWith('*/'))
                ) {
                  // More robust: check if there are any non-comment tokens on the line
                  const tokensOnLine = sourceCode.getTokens(body, {
                    filter: token =>
                      token.loc.start.line === i + 1 &&
                      token.loc.end.line === i + 1, // i is 0-based, line is 1-based
                    includeComments: false, // We only want code tokens
                  });
